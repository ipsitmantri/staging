...
4. **Generating the largest Augmented Model**: For a particular network, rather than varying a single network hyperparameter(ie: network depth,width), what if instead we vary all relevant network hyperparameters for every augmented sub-model? To advocate this it's sensible to compare the entire distribution of hyperparameter across the model. 
